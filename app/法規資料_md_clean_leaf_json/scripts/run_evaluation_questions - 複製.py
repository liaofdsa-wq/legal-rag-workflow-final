...
RAG_RUNTIME_COPY_PATH = ROOT / "rag_runtime - 複製.py"
_rag_spec = importlib.util.spec_from_file_location("rag_runtime_copy", RAG_RUNTIME_COPY_PATH)
if _rag_spec is None or _rag_spec.loader is None:
    raise ImportError(f"Unable to load rag runtime copy from {RAG_RUNTIME_COPY_PATH}")
rag_runtime_copy = importlib.util.module_from_spec(_rag_spec)
_rag_spec.loader.exec_module(rag_runtime_copy)

DEFAULT_MAX_CONTEXT_CHARS = rag_runtime_copy.DEFAULT_MAX_CONTEXT_CHARS
DEFAULT_MODEL = rag_runtime_copy.DEFAULT_MODEL
DEFAULT_OLLAMA_MODEL = rag_runtime_copy.DEFAULT_OLLAMA_MODEL
DEFAULT_OLLAMA_TIMEOUT = rag_runtime_copy.DEFAULT_OLLAMA_TIMEOUT
DEFAULT_PROMPT_TOP_N = rag_runtime_copy.DEFAULT_PROMPT_TOP_N
DEFAULT_TOP_K = rag_runtime_copy.DEFAULT_TOP_K
build_eval_prompt = rag_runtime_copy.build_eval_prompt
load_bm25_index = rag_runtime_copy.load_bm25_index
load_embedding_data = rag_runtime_copy.load_embedding_data
load_model = rag_runtime_copy.load_model
prepare_prompt_contexts = rag_runtime_copy.prepare_prompt_contexts
run_search = rag_runtime_copy.run_search


EVALUATION_DATASET_DIR = ROOT / "evaluation_dataset"
OUTPUT_PATH = ROOT / "data" / "evaluation_outputs" / "eval_outputs_local_copy.jsonl"
DEFAULT_RETRIEVAL_MODE = "combined_leaf_table"
